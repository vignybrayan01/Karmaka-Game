...
public class Partie implements Serializable {
	...
	Joueur Karadoc = new Joueur("Karadoc",TypeJoueur.Réel);
	Joueur Perceval = new Joueur("Perceval",TypeJoueur.Réel);
					...
					if(joueurCourant.type==TypeJoueur.Réel) {
						j= lireNombreDansMenu("\n Voulez vous la carte du joueur precedent ?  : \n", menu1, 1, 2, scanner);
					        ...
					        int max = 2;
					         ...
					         //fin random
					         System.out.println("Le joueur virtuel a jouer");
					}
			switch(j) {
		...
		if(joueurCourant.getPile().isEmpty()==false) {
			...
			menu.add("Sauvegarde");
					...
					if(joueurCourant.type==TypeJoueur.Réel) {
						...
						if(choix==3) { bataille.sauvegarde(bataille);break;}
			...
			menu.add("Sauvegarde");
						...
						if(choix==2) { bataille.sauvegarde(bataille);break;}				
		}
