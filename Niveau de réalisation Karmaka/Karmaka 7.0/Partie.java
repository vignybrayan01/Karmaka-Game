...
public class Partie  implements Serializable {
	...
	// création d'une partie de Bataille
		ArrayList<String> menuRV=new ArrayList<String>();
		menuRV.add("Reel");
		menuRV.add("Virtuel");
	...
	while(bataille.estTerminer==false) {
		...
		if(joueurCourant.getPile().isEmpty()==false) {
			...
			if(choix==1) {
			 ...
			 menu.add("Sauvegarde");
			 choix= lireNombreDansMenu("Que voulez vous faire : \n", menu, 1, 2, scanner);
			 if(choix==2) { Sauvegarde.sauvegarderPartie(joueurCourant, "save.dat");break;}
		}
		
		
		
		if(choix==3) {
			Sauvegarde.sauvegarderPartie(joueurCourant, "save.dat");
		}
		if(choix==1) {
