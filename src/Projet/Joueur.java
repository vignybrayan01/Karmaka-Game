...
public class Joueur implements Serializable{
	...
	public void seReincarner() {
		...
		int n= pointj + this.Jeton - (this.getNiveau().ordinal()+4);
    	...
    	if(Partie.tasCartes.size()<11) {
    		
    		Partie.tasCartes.addAll(Partie.fosse);
    		Partie.fosse.clear();
    	}
		...
		if(n<0) {
			...
			int k=pointj;
			int p=0;
			if(pointj<this.getNiveau().ordinal()+4){
				
				while(k<this.getNiveau().ordinal()+4) {
					
					k++;
					p++;
				}
				
				this.Jeton=this.Jeton-p;
				
			}
			this.niveau = niveaux[this.getNiveau().ordinal()+1];	
		}
			
		System.out.print( this.nom +"  Vous vous êtes réincarner \n");
